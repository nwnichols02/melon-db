...
5. Tap **Run jsi-sync + turbo** — uses separate bench DB files; does not touch the main app database.
6. Tap **Share JSON report** to export timings and parity ratios.
...
Parity JSON uses `ratio = jsiSyncMs / turboMs` (same shape as Node `buildRnParityReport`). See [sample device run](/docs/performance-comparison/latest-results#react-native-sample) when committed.
...
- WatermelonDB is not run on RN in this harness (Node `bench:compare` remains the WDB baseline).
