...
Pre-GA work required before publishing `@melon/*` to npm and opening the GitHub repository. **Not started** — tracked here as explicit product/engineering steps (not implied by “shipped” library code).

| Requirement | Status | Notes |
|-------------|--------|-------|
| GitHub: private → public | Gap | Repo visibility, default branch protection, required CI checks |
| Community files | Gap | `LICENSE`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md` |
| Secret / history hygiene | Gap | Audit history and CI for leaked tokens before going public |
| npm org `@melon` | Gap | Create org, invite maintainers, enforce **2FA** on all publishers |
| Per-package publish metadata | Gap | `name`, `version`, `files`, `exports`, `publishConfig.access: public` on each publishable package |
| Monorepo publish workflow | Gap | Root is `"private": true`; need CI (or manual) release that builds + publishes packages in dependency order |
| Alpha versioning policy | Gap | e.g. `0.x` semver, `alpha` dist-tag, documented breaking-change tolerance |
| npm provenance / trusted publish | Gap | Prefer OIDC or automation token with least privilege; enable provenance where supported |
| Dependency & supply-chain security | Gap | `bun audit` / npm audit in CI; review native module deps (`better-sqlite3`, RN native) |
| Security disclosure process | Gap | `SECURITY.md` with supported versions and report channel |
| Release notes & changelog | Gap | Per-package or monorepo `CHANGELOG.md`; GitHub Releases for alpha tags |
| Install docs for consumers | Gap | Document `npm install @melon/db@alpha` (or Bun) outside the monorepo |
| Support / SLA for alpha | Gap | Explicit “no production SLA”; issue templates for bug vs feature |
...
2. **npm organization & package prep**
   - Create npm org **`@melon`**; add maintainers with 2FA.
...
3. **Alpha release**
   - Publish with dist-tag **`alpha`** (e.g. `@melon/db@0.1.0-alpha.0`).
...
| Alpha / open-source release | **Next (Phase 34)** | Public GitHub + npm `@melon` alpha; see [Alpha release and open source](/docs/prd-compliance#alpha-release-and-open-source-prd-1) |
