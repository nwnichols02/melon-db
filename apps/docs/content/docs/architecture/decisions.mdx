...
**Decision:** Engine **ChangeEmitter** re-runs queries on collection changes. Native `observeQuery` / SQLite triggers deferred to Phase 27+.

**Pros:** Works on all adapters today; React hooks ship without native trigger work.

**Cons:** Coarser invalidation than row-level triggers; extra re-fetch work on large tables.
