...
```mermaid
flowchart TD
  Start["Need SQLite on RN"]
  ExpoGo{"Running in Expo Go?"}
  DevBuild{"Custom dev / release build?"}
  ExpoPath["@melon/db-sqlite/expo"]
  RnExport["@melon/db-sqlite/rn"]
  AutoMode["mode: auto default"]
  JsiSync["jsi-sync global.melonSqliteJsi"]
  Turbo["TurboModule async fallback"]

  Start --> ExpoGo
  ExpoGo -->|yes| ExpoPath
  ExpoGo -->|no| DevBuild
  DevBuild -->|yes| RnExport
  DevBuild -->|no| ExpoPath
  RnExport --> AutoMode
  AutoMode --> JsiSync
  AutoMode -->|JSI unavailable| Turbo
```
...
- [`apps/playground-rn`](https://github.com/melon/melon/tree/main/apps/playground-rn) — Expo Go
- [`apps/playground-rn-dev`](https://github.com/melon/melon/tree/main/apps/playground-rn-dev) — native JSI dev build
...
flowchart TB
  subgraph js [JavaScript]
    RnTs["@melon/db-sqlite/rn"]
    Bridge["MelonSQLiteBridge.ts"]
    JsiTs["MelonSQLiteJsi.ts"]
  end
  subgraph native [Native]
    TurboSpec["NativeMelonSQLite.ts codegen"]
    TurboIOS["iOS MelonSQLiteTurboModule.mm"]
    TurboAndroid["Android MelonSQLiteModule.kt"]
    CppHost["cpp/MelonSQLiteHostObject.cpp"]
    DbThread["Native DB worker thread"]
  end
  RnTs --> Bridge
  Bridge --> TurboSpec
  Bridge --> JsiTs
  JsiTs --> CppHost
  TurboSpec --> TurboIOS
  TurboSpec --> TurboAndroid
  TurboIOS --> CppHost
  TurboAndroid --> CppHost
  CppHost --> DbThread
```
...
- No native `observeQuery` / SQLite triggers (engine ChangeEmitter used — Phase 27+)
