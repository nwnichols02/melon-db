...
export function useQuery<T = Record<string, unknown>>(
  ...
  useEffect(() => {
    ...
    return handle.observe((value) => {
      const next = options?.select ? (options.select(value as T[]) as T[]) : (value as T[]);
      setRows(next);
    });
 ...
 */
export function useCollection<T = Record<string, unknown>>(name: string): MelonCollection<T> {
  const db = useDatabase();
 ...
 */
export function useWriter(): <T>(fn: (tx: WriteContext) => Promise<T>) => Promise<T> {
  const db = useDatabase();
