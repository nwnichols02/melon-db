...
import type { PrismaFindManyArgs, PrismaLikeClient, PrismaModelClient } from './types.ts';
...
export function createPrismaLikeClient(db: MelonDatabase): PrismaLikeClient {
  ...
  for (const name of Object.keys(db.schema.collections)) {
    const model: PrismaModelClient = {
      async findMany(args?: PrismaFindManyArgs): Promise<Record<string, unknown>[]> {
        const prepared = compilePrismaQuery(name, args, db.schema);
        return db.collection(name).findMany(prepared.ast) as Promise<Record<string, unknown>[]>;
      },

      async findFirst(args?: PrismaFindManyArgs): Promise<Record<string, unknown> | null> {
        const prepared = compilePrismaQuery(name, args, db.schema, 'one');
        return db.collection(name).findFirst(prepared.ast) as Promise<Record<string, unknown> | null>;
      },
      ...
      async delete(args: { where: Record<string, unknown> }): Promise<Record<string, unknown> | null> {
        const first = await model.findFirst({ where: args.where });
        if (!first || typeof first !== 'object' || !('id' in first)) {
          return null;
        }
