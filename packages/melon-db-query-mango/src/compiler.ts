...
function compileSelector(selector: MangoSelector): QueryBooleanNode {
  ...
  for (const [key, value] of Object.entries(selector)) {
    ...
    if (key === '$not' && typeof value === 'object' && value !== null) {
      nodes.push({ type: 'not', node: compileSelector(value as MangoSelector) });
      continue;
    }
  ...
  if (nodes.length === 0) {
    throw new MelonError('Empty Mango selector', { code: MelonErrorCode.QUERY_INVALID });
  }
...
export interface MangoQueryCompiler {
  ...
  compile(query: MangoQuery, collection: string, schema: MelonSchema): PreparedQuery;
}
