...
import { MelonError } from "../src/errors.js";
import { createMelonSchema } from "../src/schema.js";
import { adapterCrudSchemaDefinition, filterSortLimitQuery, filterSortLimitSeeds, insertFindUpdateDeleteSeed, } from "./adapter-crud-vectors.js";
/**
 * Runs shared CRUD parity scenarios against any StorageAdapter factory.
 */
export async function runAdapterCrudVectors(createAdapter) {
...
async function runInsertFindUpdateDelete(db) {
...
async function runFilterSortLimit(db) {
    await db.write(async (tx) => {
...
async function runWriteGuard(db) {
    try {
...
async function runSerializedWrites(db) {
    const order = [];
    await Promise.all([
        db.write(async (tx) => {
            order.push(1);
    ...
    if (order[0] !== 1 || order[1] !== 2) {
        throw new Error(`Expected serialized writes [1, 2], got [${order.join(", ")}]`);
    }
}
//# sourceMappingURL=run-adapter-crud-vectors.js.map