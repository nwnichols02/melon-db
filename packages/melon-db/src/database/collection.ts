...
export function createCollection<RecordShape = Record<string, unknown>>(
  ...
  const { adapter, schema, metadata, emitter, devtools, isInsideWrite, ensureReady } = deps;
  ...
  function toHandle(query?: QueryAst) {
    ...
    return createQueryHandle<RecordShape>({ adapter, emitter, devtools, ensureReady }, prepared);
  }
  ...
  return {
    ...
    async findById(id: string | number): Promise<RecordShape | null> {
      const handle = toHandle({
        collection: name,
        mode: 'one',
        where: { type: 'predicate', predicate: { field: metadata.primaryKey, op: 'eq', value: id } },
      });
    ...
    async findFirst(query?: QueryAst): Promise<RecordShape | null> {
      ...
      return toHandle({ ...ast, collection: name, mode: 'one', limit: 1 }).fetchOne();
    },
    ...
    async insert(data: InsertInput<RecordShape>): Promise<RecordShape> {
      ...
      const id = (data as Record<string, unknown>)[pk] as string | number | undefined;
    ...
    async update(id: string | number, data: UpdateInput<RecordShape>): Promise<RecordShape> {
      assertWriteContext();
      const op: AdapterWriteOperation = {
        type: 'update',
        collection: name,
