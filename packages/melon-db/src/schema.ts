...
export type MelonScalar = 'string' | 'number' | 'boolean' | 'date' | 'json' | 'bytes';
...
export interface MelonSchema<Collections extends Record<string, CollectionMetadata> = Record<string, CollectionMetadata>> {
  readonly version: number;
  readonly collections: Collections;
  getCollection<Name extends keyof Collections & string>(name: Name): Collections[Name];
}
...
export function createMelonSchema(def: DatabaseSchemaDefinition): MelonSchema {
  ...
  for (const [key, collection] of Object.entries(def.collections)) {
    if (collection.name !== key) {
      throw new MelonError(`Collection key "${key}" does not match collection.name "${collection.name}"`, {
        code: MelonErrorCode.SCHEMA_INVALID,
      });
    }
    ...
    if (!(primaryKey in collection.fields) && primaryKey !== 'id') {
      throw new MelonError(`Primary key "${primaryKey}" is not defined on collection "${collection.name}"`, {
        code: MelonErrorCode.SCHEMA_INVALID,
      });
    }
