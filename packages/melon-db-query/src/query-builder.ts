import type { QueryAst, QueryBooleanNode, QueryOperator, QueryRelationInclude, QuerySort } from '@melon/db';
...
export class QueryBuilder<RecordShape = Record<string, unknown>> {
  ...
  where(field: keyof RecordShape & string, op: QueryOperator, value?: unknown): this {
  ...
  and(group: (q: QueryBuilder<RecordShape>) => QueryBuilder<RecordShape>): this {
    const nested = new QueryBuilder<RecordShape>(this.collection);
    group(nested);
    const node = nested.whereNode;
    if (node) {
      this.whereNode = this.whereNode ? { type: 'and', nodes: [this.whereNode, node] } : node;
    }
    return this;
  }
  ...
  or(group: (q: QueryBuilder<RecordShape>) => QueryBuilder<RecordShape>): this {
    ...
    const node = nested.whereNode;
    if (node) {
      this.whereNode = this.whereNode ? { type: 'or', nodes: [this.whereNode, node] } : node;
    }
    return this;
  }

  orderBy(field: keyof RecordShape & string, direction: 'asc' | 'desc' = 'asc'): this {
  ...
  include(relation: keyof RecordShape & string, config?: Omit<QueryRelationInclude, 'relation'>): this {
  ...
  toAst(mode?: QueryAst['mode']): QueryAst {
    ...
    return {
      ...
      select: Object.keys(this.includes).length > 0 ? { include: this.includes } : undefined,
      mode: this.mode,
    };
  }
}
