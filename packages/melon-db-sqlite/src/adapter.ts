...
function toSqlParams(params: unknown[]): SQLQueryBindings[] {
	return params.map(serializeBinding) as SQLQueryBindings[];
}

function serializeBinding(value: unknown): SQLQueryBindings {
...
export function createSqliteAdapter(
	...
	return {
		...
		async find(query: PreparedQuery): Promise<AdapterFindResult> {
			...
			const rows = stmt.all(...toSqlParams(compiled.params)) as Record<
				string,
				unknown
			>[];
			return { rows };
		...
		async count(query: PreparedQuery): Promise<AdapterCountResult> {
			...
			lastQueryDebug = { sql: compiled.sql, params: compiled.params };
			const stmt = sqlite.query(compiled.sql);
			const row = stmt.get(...toSqlParams(compiled.params)) as {
				count: number;
			};
		...
		async write(operation: AdapterWriteOperation): Promise<void> {
			...
			if (operation.type === "insert") {
				...
				sqlite
					.query(sql)
					.run(...toSqlParams(keys.map((k) => operation.values[k])));
				return;
			}
			...
			if (operation.type === "delete") {
				...
				sqlite.query(sql).run(serializeBinding(operation.id));
			}
		},
